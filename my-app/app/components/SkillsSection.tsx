  ...
  return (
    <div ref={ref} className="mb-6">
      <div className="flex justify-between mb-2">
        ...
        <span className="text-sm text-[var(--text-secondary)] font-semibold">{level}%</span>
      </div>
    ...
    <section id="skills" className="relative py-32 bg-[var(--bg-secondary)]">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <ScrollReveal className="text-center mb-20">
          <p className="text-sm font-semibold tracking-[0.3em] uppercase text-[var(--text-secondary)] mb-4">Expertise</p>
        ...
        <div className="grid md:grid-cols-2 gap-16">
          <ScrollReveal>
            <h3 className="text-xl font-bold mb-8 flex items-center gap-3"><span className="w-8 h-px bg-[var(--border-accent)]" />Core Skills</h3>
            {skills.map((s) => <SkillBar key={s.name} name={s.name} level={s.level} />)}
          </ScrollReveal>

          <ScrollReveal delay={200}>
            <h3 className="text-xl font-bold mb-8 flex items-center gap-3"><span className="w-8 h-px bg-[var(--border-accent)]" />Toolkit</h3>
            ...
            <h3 className="text-xl font-bold mt-12 mb-8 flex items-center gap-3"><span className="w-8 h-px bg-[var(--border-accent)]" />My Process</h3>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={item.step} className="p-5 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)] hover:border-[var(--border-accent)] transition-all duration-300">
                  <span className="text-2xl font-black text-[var(--text-primary)]">{item.step}</span>
