    ...
    <section id="testimonials" className="relative py-32">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <ScrollReveal className="text-center mb-20">
          <p className="text-sm font-semibold tracking-[0.3em] uppercase text-[var(--text-secondary)] mb-4">Testimonials</p>
          ...
          {testimonials.map((t, i) => (
            <ScrollReveal key={t.name} delay={i * 150}>
              <div className="testimonial-card h-full flex flex-col">
                {/* Quote icon */}
                <span className="text-4xl text-[var(--text-muted)] font-serif mb-4">&ldquo;</span>
                <p className="text-[var(--text-secondary)] leading-relaxed mb-8 flex-1">{t.quote}</p>
                <div className="flex items-center gap-4">
                  <div className="w-11 h-11 rounded-full bg-[var(--bg-card-hover)] border border-[var(--border-accent)] flex items-center justify-center text-sm font-bold text-[var(--text-primary)]">{t.avatar}</div>
                  <div>
